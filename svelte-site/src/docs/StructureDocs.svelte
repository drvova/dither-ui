<script lang="ts">
  ...
  let variantTab = $state("Overview")
  let vertTab = $state("Inbox")
  const VERT_TABS: { value: string; badge?: number; disabled?: boolean }[] = [
...
<DitherTabs bind:value={tab} tabs={["Overview", "Metrics", "Logs"]}>
...
<DitherTabs bind:value={tab} {tabs} variant="segmented" />

<!-- objects add badges and disabled; vertical flips the rail -->
<DitherTabs bind:value={folder} orientation="vertical" variant="washed"
  tabs={[{ value: "Inbox", badge: 12 }, { value: "Spam", disabled: true }]} />`

  const SNIPPET_COLLAPSIBLE = `<script lang="ts">
  let openA = $state(true)
  let openB = $state(false)
<\/script>

<DitherCollapsible bind:value={openA} title="What is dithering?" color="blue">
  Ordered dithering trades smooth gradients for a fixed threshold
  matrix — the same Bayer 4x4 behind every fill in this kit.
</DitherCollapsible>
<DitherCollapsible bind:value={openB} title="Why canvas?" color="purple">
  One engine paints every fill, so components stay coherent.
</DitherCollapsible>`

  const SNIPPET_DIALOG = `<script lang="ts">
  let open = $state(false)
<\/script>

<DitherButton onclick={() => (open = true)}>Open dialog</DitherButton>
<DitherDialog open={open} title="Confirm"
  description="Ship the dithered build to production?" onclose={() => (open = false)}>
...
<section id="tabs" class="mt-20 scroll-mt-20">
  ...
  <DemoCard code={SNIPPET_TABS}>
    <div class="mx-auto max-w-sm">
      <DitherTabs bind:value={tab} tabs={TABS} color="blue">
        {#each TABS as t (t)}
          <DitherTabPanel value={t} class="mt-4">
  ...
  <div class="mt-4 grid gap-5 sm:grid-cols-3">
    {#each TAB_VARIANTS as v (v)}
      <div>
        <div class="flex h-16 items-start justify-center rounded-lg border border-border/60 p-3">
          <DitherTabs bind:value={variantTab} tabs={TABS} variant={v} color="purple" />
        </div>
  ...
  <div class="mt-4 rounded-lg border border-border/60 p-4">
    <DitherTabs bind:value={vertTab} tabs={VERT_TABS} orientation="vertical" variant="washed" color="green" class="mx-auto max-w-sm">
      {#each VERT_TABS as t (t.value)}
        <DitherTabPanel value={t.value} class="min-w-0 flex-1 self-stretch rounded-md border border-border/40 p-3">
...
<section id="collapsible" class="mt-20 scroll-mt-20">
...
<section id="dialog" class="mt-20 scroll-mt-20">
  ...
  <DemoCard code={SNIPPET_DIALOG}>
    <div class="flex justify-center">
      <DitherButton onclick={() => (dialogOpen = true)}>Open dialog</DitherButton>
      <DitherDialog open={dialogOpen} title="Confirm" description="Ship the dithered build to production?" onclose={() => (dialogOpen = false)}>
        <p class="text-[13px] leading-relaxed text-muted-foreground">Review the release before continuing.</p>
        {#snippet footer()}
          <DitherButton color="green" onclick={() => (dialogOpen = false)}>Confirm</DitherButton>
        {/snippet}
      </DitherDialog>
    </div>
  </DemoCard>
...
<section id="kbd" class="mt-20 scroll-mt-20">

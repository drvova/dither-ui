...
<template>
  <header class="flex h-12 items-center justify-between border-b border-border/60 bg-background px-4">
    ...
    <div class="flex items-center gap-1.5">
      <!-- Add -->
      <div class="relative">
        <button type="button" class="flex items-center gap-1.5 rounded-md bg-foreground px-2.5 py-1.5 text-[11px] text-background transition-opacity hover:opacity-90" @click="addOpen = !addOpen">
          + artboard
        </button>
      ...
      <button type="button" :disabled="!canEdit()" class="rounded-md border border-border px-2.5 py-1.5 text-[11px] text-muted-foreground transition-colors hover:text-foreground disabled:opacity-40" @click="duplicateSelected">duplicate</button>
      <button type="button" :disabled="!canEdit()" class="rounded-md border border-border px-2.5 py-1.5 text-[11px] text-muted-foreground transition-colors hover:text-foreground disabled:opacity-40" @click="removeSelected">delete</button>

      <span class="mx-1 h-4 w-px bg-border" />

      <button type="button" class="flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-[11px] text-muted-foreground transition-colors hover:text-foreground" @click="replay">
      ...
      <button type="button" class="rounded-md border border-border px-2.5 py-1.5 text-[11px] text-muted-foreground transition-colors hover:text-foreground" @click="emit('export')">export</button>

      <button type="button" class="flex size-8 items-center justify-center rounded-md border border-border text-muted-foreground transition-colors hover:text-foreground" :aria-label="dark ? 'Light' : 'Dark'" @click="toggle">

<script lang="ts" module>
  ...
  const CLOSE = "cubic-bezier(0.3, 0, 0.2, 1)"
...
<script lang="ts">
  ...
  type Props = {
    items: BouncyItem[]
    /** Open item value (bindable); "" closes all. */
    value?: string
    color?: PixelColor
    class?: string
    // Per-item panel content arrives as named snippet props keyed by item value
    // (Vue's `<slot :name="item.value" />`); item.content is the fallback.
    [slot: string]: unknown
  }

  let { items, value = $bindable(""), color = "blue", class: className, ...slots }: Props = $props()
  ...
  const toggle = (v: string) => (value = isOpen(v) ? "" : v)

  // Init = resting height; update = spring toward the new state when the open
  // flag flips (the action's reactive-param seam).
  function springOpen(node: HTMLElement, params: { open: boolean }) {
    node.style.height = params.open ? "auto" : "0px"
    let was = params.open
    return {
      update(next: { open: boolean }) {
        if (next.open === was) return
        was = next.open
        if (still) {
          node.style.height = next.open ? "auto" : "0px"
          return
        }
        springHeight(node, next.open ? node.scrollHeight : 0, next.open)
      },
    }
  }
</script>
    ...
    <div class="border-t border-border/40 first:border-t-0">
      ...
      >
        ...
        <span class="min-w-0 flex-1">
          ...
          {#if item.hint}
            <span class="block truncate text-[11px] text-muted-foreground">{item.hint}</span>
          {/if}
        </span>
        <span
          aria-hidden="true"
          class="text-muted-foreground transition-transform duration-200 motion-reduce:transition-none {isOpen(item.value)
            ? 'rotate-90'
            : ''}"
        >
          ›
      ...
      <div
        id={`${id}-${i}`}
        use:springOpen={{ open: isOpen(item.value) }}
      ...
      >
        <div
          ...
          style:opacity={isOpen(item.value) ? "1" : "0"}
          style:transform={isOpen(item.value) ? "none" : "translateY(-6px)"}
          style:transition-timing-function={isOpen(item.value) ? SPRING : CLOSE}
          style:transition-duration={isOpen(item.value) ? "560ms" : "240ms"}
        >
          {#if slotFor(item.value)}

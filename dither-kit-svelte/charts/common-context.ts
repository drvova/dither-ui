 ...
 * a template/derived tracks reactively.
 */
...
const CommonChartKey = Symbol("dither-common-chart")

/** Published by every chart root alongside its family-specific context. */
export function setCommonChart(ctx: CommonChart): void {
  setContext(CommonChartKey, ctx)
}

export function useCommonChart(): CommonChart {
  const ctx = getContext<CommonChart>(CommonChartKey) ?? null
  if (!ctx) {

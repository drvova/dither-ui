<script lang="ts" module>
  import { paintBallpit, type BallpitParams } from "../engine/ballpit"
  export { paintBallpit }
...
<script lang="ts">
  ...
  const params = $derived<BallpitParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintBallpit(buffer, params, clock, matrix, mouseInteraction ? mouse : null),
  })

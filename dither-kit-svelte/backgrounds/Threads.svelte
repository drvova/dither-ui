<script lang="ts" module>
  import { paintThreads, type ThreadsParams } from "../engine/threads"
  export { paintThreads }
...
<script lang="ts">
  ...
  const params = $derived<ThreadsParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintThreads(buffer, params, clock, matrix),
  })
</script>

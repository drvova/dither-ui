<script lang="ts" module>
  import { paintIridescence, type IridescenceParams } from "../engine/iridescence"
...
<script lang="ts">
  ...
  const params = $derived<IridescenceParams>({
    scale: scale,
    speed: speed,
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintIridescence(buffer, params, clock, matrix),
  })
</script>

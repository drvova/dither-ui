<script lang="ts" module>
  import { paintRibbons, type RibbonsParams } from "../engine/ribbons"
  export { paintRibbons }
...
<script lang="ts">
  ...
  const params = $derived<RibbonsParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintRibbons(buffer, params, clock, matrix, mouseInteraction ? mouse : null),
  })

<script lang="ts" module>
  import { paintParticles, type ParticlesParams } from "../engine/particles"
  export { paintParticles }
...
<script lang="ts">
  ...
  const params = $derived<ParticlesParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintParticles(buffer, params, clock, matrix, mouseInteraction ? mouse : null),
  })

<script lang="ts" module>
  import { paintGradientBlinds, type GradientBlindsParams } from "../engine/gradient-blinds"
...
<script lang="ts">
  ...
  const params = $derived<GradientBlindsParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintGradientBlinds(buffer, params, clock, matrix),
  })
</script>

<script lang="ts" module>
  import { paintSilk, type SilkParams } from "../engine/silk"
  export { paintSilk }
...
<script lang="ts">
  ...
  const params = $derived<SilkParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintSilk(buffer, params, clock, matrix),
  })
</script>

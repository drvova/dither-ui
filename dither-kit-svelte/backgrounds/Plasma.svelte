<script lang="ts" module>
  import { paintPlasma, type PlasmaParams } from "../engine/plasma"
  export { paintPlasma }
...
<script lang="ts">
  ...
  const params = $derived<PlasmaParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintPlasma(buffer, params, clock, matrix),
  })
</script>

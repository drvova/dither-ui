<script lang="ts" module>
  import { paintGridDistortion, type GridDistortionParams } from "../engine/grid-distortion"
  export { paintGridDistortion }
...
<script lang="ts">
  ...
  const params = $derived<GridDistortionParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintGridDistortion(buffer, params, clock, matrix, mouseInteraction ? mouse : null),
  })

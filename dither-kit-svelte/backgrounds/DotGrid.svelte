<script lang="ts" module>
  import { paintDotGrid, type DotGridParams } from "../engine/dot-grid"
  export { paintDotGrid }
...
<script lang="ts">
  ...
  const params = $derived<DotGridParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintDotGrid(buffer, params, clock, matrix, mouseInteraction ? mouse : null),
  })

<script lang="ts" module>
  import { paintBeams, type BeamsParams } from "../engine/beams"
  export { paintBeams }
...
<script lang="ts">
  ...
  const params = $derived<BeamsParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintBeams(buffer, params, clock, matrix),
  })
</script>

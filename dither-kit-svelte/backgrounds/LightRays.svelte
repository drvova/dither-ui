<script lang="ts" module>
  import { paintRays, type RaysParams } from "../engine/rays"
  export { paintRays }
...
<script lang="ts">
  ...
  const params = $derived<RaysParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintRays(buffer, params, clock, matrix),
  })
</script>

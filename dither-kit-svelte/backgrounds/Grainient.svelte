<script lang="ts" module>
  import { paintGrainient, type GrainientParams } from "../engine/grainient"
...
<script lang="ts">
  ...
  const params = $derived<GrainientParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintGrainient(buffer, params, clock, matrix),
  })
</script>

<script lang="ts" module>
  import { paintAurora, type AuroraParams } from "../engine/aurora"
  export { paintAurora }
...
<script lang="ts">
  ...
  const params = $derived<AuroraParams>({
  ...
  const bg = $derived({
    ...
    render: (buffer: RasterBuffer, clock: number) => paintAurora(buffer, params, clock, matrix),
  })
</script>
